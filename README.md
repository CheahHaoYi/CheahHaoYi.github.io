...
```
        ...
        ┌───────────────────┬────────────┴───────────┬───────────────────┐
        ▼                   ▼                        ▼                   ▼
  /cissp               /ccna                    /rhcsa              /projects
(CISSP Notes)        (CCNA Notes)             (RHCSA Notes)       (Projects Hub)
```
...
3. **Shallow Hub Content**: This repository does *not* host deep certification study notes or full project write-ups. It hosts high-level biographies, timelines, teaser cards, and plain absolute links (`/cissp`, `/ccna`, `/rhcsa`, `/projects`) to sibling properties.
...
| `/about` | **About & Philosophy**: Narrative on background (NUS Computer Engineering, DTU Exchange), security domain synergy matrix (CISSP, CCNA, RHCSA, AI), and academic coursework. | `<Layout>`, `<Table>`, `<Prose>`, `<Card>` |
...
- [`src/data/knowledgeAreas.ts`](src/data/knowledgeAreas.ts): Registry of certification domains and sub-sites (`CISSP`, `CCNA`, `RHCSA`, `AI Security`). Adding a new certification portal (e.g. `CCNP`, `OSCP`) requires editing this single file.
